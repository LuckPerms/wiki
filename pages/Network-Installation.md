...
* If you're running multiple servers which are not connected to one proxy that runs LuckPerms Bungee/Velocity, but use the same database set this option to `sql`.
* If you're running a small network with one BungeeCord/Velocity proxy that runs LuckPerms Bungee/Velocity, then you should set this option to `pluginmsg`.
